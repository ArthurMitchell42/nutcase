...
# Send_APC_Log_Reply
#=======================================================================
def Send_APC_Log_Reply( Channel, Scrape_Lines ):    
    Formatted_JSON_Text = json.dumps( Scrape_Lines )
...
# Send_JSON_Reply
#=======================================================================
def Send_JSON_Reply( Channel, Scrape_Data ):    
...
# Send_Raw_JSON_Reply
#=======================================================================
def Send_Raw_JSON_Reply( Channel, Scrape_Data ):
...
def Send_Usage_Reply( Channel, Error_404 ):
    ...
    Channel.wfile.write(bytes("<span "+server_constants.Monospace_Font+"><b>/metrics?target=A.B.C.D:P</b></span>  Specifying the address of the server and the port.<br>", "utf-8"))
    ...
    Channel.wfile.write(bytes("<span "+server_constants.Monospace_Font+"><b>/json?target=A.B.C.D:P</b></span>  Specifying the address of the server and the port.<br>", "utf-8"))
    ...
    Channel.wfile.write(bytes("<span "+server_constants.Monospace_Font+"><b>/raw?target=A.B.C.D:P</b></span>  Specifying the address of the server and the port.<br>", "utf-8"))
    ...
    Channel.wfile.write(bytes("</span><span "+server_constants.HTML_Colour_Blue+">", "utf-8"))
    Channel.wfile.write(bytes("<span "+server_constants.Monospace_Font+"><p style='margin-left: 20px;'><b>http://10.0.20.40:9995/metrics?target=10.0.20.50:3493</b></span></p>", "utf-8"))
...
def Send_Log_Reply( Channel, Data ):
    ...
    try:
        File_Handle = open( globals.Log_File, 'r')
        Channel.send_response(200)
        Channel.send_header(keyword="Content-type", value=server_constants.Content_Types["html"])
        Channel.end_headers()
        ...
        Channel.wfile.write(bytes("<p>Could not open log file: %s</p>" % globals.Log_File, "utf-8"))
        Channel.wfile.write(bytes("</body></html>", "utf-8"))
    return
...
#====================================================================================================
def Validate_Target( Param , Target_Port ):
...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if "mode" in params:
            if params["mode"][0].lower() == "nut":
                Server_Type = Server_Protocol.NUT
            elif params["mode"][0].lower() == "apc":
                Server_Type = Server_Protocol.APC
            else:
                Server_Type = Server_Protocol.NONE
                log.error("Unknown mode requested {}".format( params["mode"][0] ))
        ...
        if Server_Type == Server_Protocol.NUT:
            ...
            Target_Port = 3551      # The default APC server port number, may be overridden by parameters

        if "target" in params:
            rtn, Target_Address, Target_Port = Validate_Target(params["target"][0], Target_Port)
            if rtn:
        ...
        #============================================================================================
        # Route to a responder ased on the end point path
        #============================================================================================
        if parsed_path.path == "/log":
            Send_Log_Reply( self, params )
        elif parsed_path.path == "/help":
            Send_Usage_Reply( self, False )
        elif parsed_path.path == "/apclog":
            Scrape_Lines = apc_server_handler.Get_APC_Log(Target_Address, Target_Port)
            Send_APC_Log_Reply( self, Scrape_Lines )
        else:
            if parsed_path.path == "/metrics" or \
               parsed_path.path == "/json" or    \
               parsed_path.path == "/raw":
                ...
                if Server_Type == Server_Protocol.NUT:
                    ...
                    Send_Usage_Reply( self, True )
                    log.warning('Server_Type: Returned 404: {}'.format(parsed_path.path) ) 
                ...
                if parsed_path.path == "/metrics":
                    ...
                    if Target_Resolved:
                        Send_JSON_Reply( self, Scrape_Data )
                    else:
                        Send_Usage_Reply( self, True )
                        log.warning('Target not sent, returned 404: {}'.format(parsed_path.path) ) 
                elif parsed_path.path == "/raw":
                    if Target_Resolved:
                        Send_Raw_JSON_Reply( self, Scrape_Data )
                    else:
                        Send_Usage_Reply( self, True )
                        log.warning('Target not sent, returned 404: {}'.format(parsed_path.path) ) 
            else:
