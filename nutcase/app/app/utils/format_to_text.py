...
def Find_Metric_By_Name(Metric_Name):
    return next((sub for sub in metric_data.Metric_Data_List if sub['metric'] == Metric_Name),
                None)
...
def Format_Version_Metric(Metric, Version):
    ...
    Data["data"] = [ "{metric}{{version=\"{version}\"}} 1".format(
        metric=Metric["metric"], version=Version)]
    return Data
...
# Set_NUT_Variable
# ===================================================================================================
def Set_NUT_Variable(ups, Variable_Name, New_Value):
    Variable = next((sub for sub in ups["variables"] if sub['name'] == Variable_Name), None)
    if Variable:
...
# Remove_NUT_Variable
# ===================================================================================================
def Remove_NUT_Variable(ups, Variable_Name):
    Variable = next((sub for sub in ups["variables"] if sub['name'] == Variable_Name), None)
    if Variable:
        current_app.logger.debug("Remove_NUT_Variable: poping {}".format(
            ups['variables'].index(Variable)))
        ups['variables'].pop(ups['variables'].index(Variable))
        return True
    return False
...
# Get_UPS
# ===================================================================================================
def Get_UPS(Scrape_Data, Device):
    UPS = None

    for i in Scrape_Data["ups_list"]:
        if i["name"] == Device:
            UPS = i
            break
    return UPS
...
def Format_UPS_Info_Metric(Metric, ups):
    ...
    Data["data"] = [ "{metric}{{ups=\"{name}\",description=\"{ups_desc}\"{labels}}} 1".format(
        metric=Metric["metric"], name=ups["name"], ups_desc=ups["description"], labels=Labels)]
    return Data
...
def Format_UPS_Status_Metric(Metric, ups):
    ...
    Data["type"] = "# TYPE {metric} {style}".format(metric=Metric["metric"], style=Metric["style"])
    Data["unit"] = "# UNIT {metric} {unit}".format(metric=Metric["metric"], unit=Metric["unit"])
    Data["help"] = "# HELP {metric} {help} (\"{nut_var}\")".format(metric=Metric["metric"],
                                            help=Metric["help"], nut_var=Metric["nut_var"])
    Data["data"] = []
    ...
    for Status in UPS_Status_List:
        if Status.upper() in Status_List:
            ...
            Value = "0"
        Data["data"].append("{metric}{{ups=\"{name}\",status=\"{status}\"}} {value}".format(
            metric=Metric["metric"], name=ups["name"], status=Status, value=Value))
    return Data
...
def Format_Variable_Metric(Metric, UPS_Name, NUT_Variable):
    ...
    Data["data"] = [ "{metric}{{ups=\"{ups_name}\"}} {value}".format(metric=Metric["metric"],
                                                            ups_name=UPS_Name, value=Value)]
...
def Format_For_Prometheus(Scrape_Data):
    current_app.logger.debugv("In Format_For_Prometheus. Scrape_Data is:\n{}".format(Scrape_Data))

    # =============================================================================
    # Add system information to the list of metrics
    # =============================================================================
    Metric_List = []
    Metric_List.append(Format_Version_Metric(Find_Metric_By_Name("nut_exporter_info"),
                                             Scrape_Data["nutcase_version"]))
    Metric_List.append(Format_Version_Metric(Find_Metric_By_Name("nut_server_info"),
                                             Scrape_Data["server_version"]))
    Metric_List.append(Format_Version_Metric(Find_Metric_By_Name("nut_info"),
                                             Scrape_Data["server_version"]))
    ...
    Formatted_Text = ""
    if current_app.config['ORDER_METRICS']:   # globals.Order_Metrics:
        # =============================================================================
        ...
        # =============================================================================
        # Print the data for each metric to a list of lines in the order received
        # =============================================================================
        for Metric in Metric_List:
            Formatted_Text += Get_Metric_Text(Metric)
