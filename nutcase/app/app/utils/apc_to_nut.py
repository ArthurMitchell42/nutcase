...
# /*Extended bit values added by apcupsd
    { "Define": "UPS_commlost",    "Mask":  0x00000100, "Status": ""},   # Comms with UPS lost
    { "Define": "UPS_shutdown",    "Mask":  0x00000200, "Status": "SD"},  # Shutdown in prog
    ...
    { "Define": "UPS_onbatt_msg",  "Mask":  0x00020000, "Status": ""},   # Set when UPS_ONBATT sent
    { "Define": "UPS_fastpoll",    "Mask":  0x00040000, "Status": ""},   # Set on pfail, poll fastr
    { "Define": "UPS_shut_load",   "Mask":  0x00080000, "Status": ""},   # 1 when BatLoad<=percent
    { "Define": "UPS_shut_btime",  "Mask":  0x00100000, "Status": ""},  # 1 when time on bat>maxtim
    { "Define": "UPS_shut_ltime",  "Mask":  0x00200000, "Status": "SD"},  # 1 when TimeLeft<=runtim
    { "Define": "UPS_shut_emerg",  "Mask":  0x00400000, "Status": "FSD"},  # 1=bat power has failed
    { "Define": "UPS_shut_remote", "Mask":  0x00800000, "Status": ""},   # Set when remote shutdown
    { "Define": "UPS_plugged",     "Mask":  0x01000000, "Status": ""},   # 1=comp is plugged to UPS
    { "Define": "UPS_battpresent", "Mask":  0x04000000, "Status": ""},   # 1= battery is connected
]
...
def Time_Controls(Scrape_Data):
    # DWAKE
    # The amount of time the UPS will wait before restoring power
    # to your equipment after a power off
...
def Beeper_Status(Scrape_Data):
    ...
    Scrape_Data['ups_list'][0]['variables'].append(
        {'name': 'ups.beeper.status', 'value': BeepState})
    Scrape_Data['ups_list'][0]['variables'].append(
        {'name': 'ups.beeper.status.text', 'value': BeepStateText})
    return
...
def UPS_Status(Scrape_Data):
    ...
    for Define in Constants_STATFLAG:
        if (Define['Mask'] & Status_Word) != 0:
            current_app.logger.debugvv("Found: {} {}".format(Define['Define'],
                                                             Define['Status']))
    ...
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'ups.status',
                                                    'value': Status_String.strip()})
    return
...
def Output_Power(Scrape_Data):
    ...
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'ups.realpower.nominal',
                                                    'value': NomPower})
    return
...
def Input_Voltage(Scrape_Data):
    ...
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'input.voltage',
                                                    'value': LineV})

    OUTPUTV = format_to_text.Get_NUT_Variable(Scrape_Data['ups_list'][0], 'OUTPUTV')
    if OUTPUTV:
        OutputV = str(Strip_Numeric(OUTPUTV))
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'output.voltage',
                                                    'value': OutputV})
    ...
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'input.voltage.nominal',
                                                    'value': NomV})
    ...
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'input.transfer.low',
                                                    'value': LowTrans})
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'input.transfer.high',
                                                    'value': HiTrans})
...
def Battery_Voltage(Scrape_Data):
    ...
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'battery.voltage',
                                                    'value': BattV})

    NOMBATTV = format_to_text.Get_NUT_Variable(Scrape_Data['ups_list'][0], 'NOMBATTV')
    if NOMBATTV:
        NomBattV = str(Strip_Numeric(NOMBATTV))
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'battery.voltage.nominal',
                                                    'value': NomBattV})

    Batt_Low_Volt = str(float(NomBattV) * 0.87)
    Scrape_Data['ups_list'][0]['variables'].append({'name': 'battery.voltage.low',
                                                    'value': Batt_Low_Volt})
    return
...
def Battery_Charge(Scrape_Data):
    ...
    BCHARGE = format_to_text.Get_NUT_Variable(Scrape_Data[
                                            'ups_list'][0], 'BCHARGE')
    MBATTCHG = format_to_text.Get_NUT_Variable(Scrape_Data[
                                            'ups_list'][0], 'MBATTCHG')
    if BCHARGE:
    ...
    Scrape_Data['ups_list'][0]['variables'].append(
                            {'name': 'battery.charge', 'value': BCharge})
    Scrape_Data['ups_list'][0]['variables'].append(
                            {'name': 'battery.charge.low', 'value': MBattChg})
    Scrape_Data['ups_list'][0]['variables'].append(
                            {'name': 'battery.charge.warning', 'value': MBattChg})
...
def Battery_Runtime(Scrape_Data):
    # DLOWBATT
    # The remaining runtime below which the UPS sends the low battery signal.
    #  At this point apcupsd will force an immediate emergency shutdown.
    # MINTIMEL
    # apcupsd will shutdown your system if the remaining runtime equals
    #   or is below this point. Value is set
    ...
    TIMELEFT = format_to_text.Get_NUT_Variable(
                        Scrape_Data['ups_list'][0], 'TIMELEFT')
    if TIMELEFT:
        TimeLeft = str(int(Strip_Numeric(TIMELEFT) * 60))
    Scrape_Data['ups_list'][0]['variables'].append(
                    {'name': 'battery.runtime', 'value': TimeLeft})

    MINTIMEL = format_to_text.Get_NUT_Variable(
                        Scrape_Data['ups_list'][0], 'MINTIMEL')
    if MINTIMEL:
        MinTimeL = str(Strip_Numeric(MINTIMEL) * 60)
    Scrape_Data['ups_list'][0]['variables'].append({
        'name': 'battery.runtime.low', 'value': MinTimeL})
    return
...
def Translate_APC_To_NUT(Scrape_Data):
    ...
    current_app.logger.debug("Translate_APC_To_NUT: Transformed Scrape_Data {}".format(
        Scrape_Data))
