...
# Enumerated types
# ======================================================================
NUT_Query_List_State = Enum('NUT_Query_List_State',
                            ['INITIAL', 'STARTED', 'ENDED', 'MALFORMED', 'ERROR'])
                ...
                if List_State == NUT_Query_List_State.INITIAL:
                    ...
                    if re.search(End_Pattern, Line):
                        ...
                        if ups_match:
                            ...
                            current_app.logger.debugv("Client match {} for UPS {}".format(
                                Client["client"], Client["upsname"]))
...
def Query_NUT_Version(Socket, Socket_File, Debug_Lines):
    ...
    if match:
        current_app.logger.error("The NUT server returned an error when asked for its version: {}".format(Version_Line))  # noqa: E501
        if match.group(1) == "ACCESS-DENIED":
            IPAddr = Socket.getsockname()[0]
            current_app.logger.error(
                "ACCESS-DENIED implies NUT svr has not had my IP address ({}) in its allowed list.".format(IPAddr))  # noqa: E501
        return False, ""
    ...
    if match:
        ...
        current_app.logger.error("Could not parse version in the received version line: {}".format(
            Version_Line))
        return False, ""
...
def Query_NUT_Variables(Socket_File, UPSs, Debug_Lines):
    for ups in UPSs:
        ...
        if not Parse_Server_Response(Socket_File, Request, UPSs, Debug_Lines):
            current_app.logger.warning("Problem encountered listing variables of UPS: {}".format(
                ups["name"]))
            return False
    return True
...
def Query_NUT_UPS_Clients(Socket_File, UPSs, Debug_Lines):
    for ups in UPSs:
        ...
        if not Parse_Server_Response(Socket_File, Request, UPSs, Debug_Lines):
            current_app.logger.warning("Problem encountered listing clients of UPS: {}".format(
                ups["name"]))
            return False
    return True
...
def Scrape_NUT_Server(Target_Address, Target_Port = 3493):
    current_app.logger.debugv("Scrape started of NUT server address {} port {}".format(
