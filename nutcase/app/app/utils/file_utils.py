...
def Tail_File( File_Name, Display_Lines=20):
    ...
    try:
        ...
        Log_Pattern = re.compile(r"^([0-9-]+) ([0-9:,]+) (DEBUGVV|DEBUGV|DEBUG|INFO|WARNING|ERROR|CRITICAL|FATAL) (.+)$")
        count = 0
        ...
        while True:
            ...
            if current_app.config["COLOURED_LOG"]:
                ...
                if match:
                    ...
                    elif match.group(3) == "DEBUGV":    Level_Colour = server_constants.HTML_Colour_Green
                    elif match.group(3) == "DEBUGVV":   Level_Colour = server_constants.HTML_Colour_Green
                    ...
                    line = '{date} {time} <span style="{lstyle}">{level}</span> {message}<br>'.format( 
                            ...
                            level  = match.group(3),    message= match.group(4),
                            lstyle = Level_Colour )
            Lines.append( line )
        ...
        current_app.logger.warning( "Could not open log file to serve it: {}".format( File_Name ) ) 
