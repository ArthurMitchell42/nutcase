...
def Remove_NUT_Variable( ups, Variable_Name ):
    Variable = next((sub for sub in ups["variables"] if sub['name'] == Variable_Name), None)
    if Variable:
        current_app.logger.debug("Remove_NUT_Variable: poping {}".format( ups['variables'].index(Variable) ))
...
def Format_For_Prometheus( Scrape_Data ):
    ...
    #==============================================================================
    Metric_List = []
    Metric_List.append(Format_Version_Metric( Find_Metric_By_Name("nut_exporter_info"), Scrape_Data["nutcase_version"] ))
