...
from app.api.api_utils import Get_Update_String, Set_Config_Update_String
    ...
    def test_gus_http_error(self, mock_urlopen):
        ...
        with self.assertLogs(logging.getLogger(__name__), level='DEBUG') as cm:
            ...
            self.assertEqual(cm.output, [
                # "WARNING:unit.test_api_utils:Failed to call Github API. Reason: Bad request",
                # "WARNING:unit.test_api_utils:Github couldn't fulfill request Error: 400",
                "WARNING:Failed to call Github API. Reason: Bad request",
                "WARNING:Github couldn't fulfill request Error: 400",
                ])
...
# URLError
    def test_gus_url_error(self, mock_urlopen):
        ...
        with self.assertLogs(logging.getLogger(__name__), level='DEBUG') as cm:
            ...
            Rtn = Get_Update_String()
            self.assertEqual(cm.output, [
                # "WARNING:tests.unit.test_api_utils:Failed to call Github API. Reason: The reason"
                "WARNING:Failed to call Github API. Reason: The reason"
                ])
        self.assertEqual(Rtn, "")
...
# Return 404
    def test_gus_return_404(self, mock_urlopen):
        ...
        with self.assertLogs(logging.getLogger(__name__), level='DEBUG') as cm:
            ...
            Rtn = Get_Update_String()
            self.assertEqual(cm.output, [
                # "WARNING:tests.unit.test_api_utils:Github returned 404: NOT_FOUND",
                "WARNING:Github returned 404: NOT_FOUND",
                ])
...
# Return duff json
    def test_gus_return_duff_json(self, mock_urlopen):
        url = self.app.config['GITHUB_API_URL'] + "releases"
        rtn_obj = Urlopen(url, read_data='error_json')
        mock_urlopen.return_value = rtn_obj
        ...
        with self.assertLogs(logging.getLogger(__name__), level='DEBUG') as cm:
            ...
            Rtn = Get_Update_String()
            self.assertEqual(cm.output, [
                # "WARNING:tests.unit.test_api_utils:JSON Returned by Github could not be parsed "
                "WARNING:JSON Returned by Github could not be parsed "
    ...
    # Pre-rel and release available using Set_Config_Update_String
    def test_gus_normal_current_update_config(self, mock_urlopen):
        self.maxDiff = None
        self.app.config.update(APP_VERSION = '0.3.0')
        url = self.app.config['GITHUB_API_URL'] + "releases"
        rtn_obj = Urlopen(url, code=HTTPStatus.OK, read_data=Test_prerel_JSON)
        ...
        self.assertEqual(self.app.config['UPDATE_HTML'], '<a href="https://github/V0.3.1_release" target="_blank" class="text-decoration-none text-small badge rounded-pill bg-warning text-dark">V0.3.1_release</a>')
        self.assertEqual(mock_urlopen.call_count, 1)
        self.assertEqual(rtn_obj.geturl(), url)
